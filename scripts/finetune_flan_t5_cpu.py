#!/usr/bin/env python3
"""
Fine-tuning google/flan-t5-small dla tłumaczenia EN→PL (dane biblijne).
...
def preprocess_function(examples, tokenizer, max_source_length: int, max_target_length: int, src_lang: str, tgt_lang: str):
    ...
    Dla flat_t5: source = EN, target = PL.
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Fine-tuning google/flan-t5-small EN->PL (dane biblijne). Działa na CPU i GPU.")
    ...
    ap.add_argument("--output-dir", type=Path, default=None, 
                    help="Katalog wyjściowy. (default: outputs/finetuned/flan_t5_small_full, nadpisuje config)")
    ...
    ap.add_argument("--resume-from-checkpoint", type=str, default=None, 
                    help="Ścieżka do checkpointu do wznowienia treningu (np. 'checkpoint-250' lub 'outputs/finetuned/flan_t5_small/checkpoint-250'). "
    ...
    model_name = str(pick(args.model, get_nested(cfg, ["finetune_flat_t5", "model_name"]), "google/flan-t5-small"))
    src_lang = str(get_nested(cfg, ["finetune_flat_t5", "src_lang"]) or "en")
    tgt_lang = str(get_nested(cfg, ["finetune_flat_t5", "tgt_lang"]) or "pl")

    # Ścieżki do danych: --data-dir ma priorytet, potem config, potem default
    data_dir = args.data_dir
    train_en = Path(pick(None, get_nested(cfg, ["finetune_flat_t5", "train_en"]), str(data_dir / "train.en")))
    train_pl = Path(pick(None, get_nested(cfg, ["finetune_flat_t5", "train_pl"]), str(data_dir / "train.pl")))
    val_en = Path(pick(None, get_nested(cfg, ["finetune_flat_t5", "val_en"]), str(data_dir / "val.en")))
    val_pl = Path(pick(None, get_nested(cfg, ["finetune_flat_t5", "val_pl"]), str(data_dir / "val.pl")))

    max_source_length = int(pick(None, get_nested(cfg, ["finetune_flat_t5", "max_source_length"]), 128))
    max_target_length = int(pick(None, get_nested(cfg, ["finetune_flat_t5", "max_target_length"]), 128))
    batch_size = int(pick(args.batch_size, get_nested(cfg, ["finetune_flat_t5", "batch_size"]), 2))
    grad_accum_steps = int(pick(None, get_nested(cfg, ["finetune_flat_t5", "grad_accum_steps"]), 8))
    learning_rate = float(pick(args.lr, get_nested(cfg, ["finetune_flat_t5", "learning_rate"]), 5.0e-5))
    num_epochs = int(pick(args.epochs, get_nested(cfg, ["finetune_flat_t5", "num_epochs"]), 1))
    warmup_ratio = float(pick(None, get_nested(cfg, ["finetune_flat_t5", "warmup_ratio"]), 0.03))
    seed = int(pick(None, get_nested(cfg, ["finetune_flat_t5", "seed"]), 2137))

    output_dir = Path(pick(args.output_dir, get_nested(cfg, ["finetune_flat_t5", "output_dir"]), "outputs/finetuned/flan_t5_small_full"))
    eval_steps = int(pick(None, get_nested(cfg, ["finetune_flat_t5", "eval_steps"]), 500))
    save_steps = int(pick(None, get_nested(cfg, ["finetune_flat_t5", "save_steps"]), 500))
    save_total_limit = int(pick(None, get_nested(cfg, ["finetune_flat_t5", "save_total_limit"]), 3))  # Domyślnie 3 checkpointy
    logging_steps = int(pick(None, get_nested(cfg, ["finetune_flat_t5", "logging_steps"]), 50))
    ...
    if quick_mode:
        ...
        output_dir = Path("outputs/finetuned/flan_t5_small_quick")
        # Wczytaj też test set dla ewaluacji
        test_en = Path(pick(None, get_nested(cfg, ["finetune_flat_t5", "test_en"]), str(data_dir / "test.en")))
        test_pl = Path(pick(None, get_nested(cfg, ["finetune_flat_t5", "test_pl"]), str(data_dir / "test.pl")))
    else:
    ...
    print("=== FINE-TUNING flan_T5-small ===")
    if quick_mode:
